...
def generate_stocks(salary, age, risk_tolerance, target_savings, allocated_stocks):
        ...
        finalStocks['Allocation']=finalStocks['Allocation'].round(2)
        return finalStocks

def calculate_annual_interest_rate(final_stocks):
    weighted_annual_return = (final_stocks['Annual Return'] * final_stocks['Allocation'] / 100).sum()
    return weighted_annual_return

def calculate_monthly_investment(target_savings, age, annual_interest_rate):
    investment_period_years = 60 - age
    investment_period_months = investment_period_years * 12
    monthly_interest_rate = (1 + annual_interest_rate) ** (1/12) - 1
    
    monthly_investment = target_savings * monthly_interest_rate / (((1 + monthly_interest_rate) ** investment_period_months - 1))
...
def allocation():
    ...
    allocated_stocks['Annual Return'] = allocated_stocks['Annual Return'] / 100

    final_stocks = generate_stocks(salary, age, risk, target_savings, allocated_stocks)

    annual_interest_rate = calculate_annual_interest_rate(final_stocks)

    monthly_investment = calculate_monthly_investment(target_savings, age, annual_interest_rate)
    monthly_investment = round(monthly_investment, 2)
    final_stocks['Invest Amount'] = (final_stocks['Allocation'] / 100) * monthly_investment
    ...
    response = {
        ...
        'allocated_per': final_stocks.to_dict(orient='records'),
        'message': 'Success',
